...
author: "Allan Chinchilla Arias - B92220"
date: "2020-05-16"
output:
  html_document:
    theme: readable    
...
```{r}
ggplot(centro_america, aes(x = name_long, y = lifeExp)) + ggtitle('Esperanza de vida en Centroamérica') + 
xlab('Paises') +
...
```{r}
plot(centro_america["lifeExp"], main = "Esperanza de vida en Centroamérica")
```
...
```{r}
#Traté de todas las maneras vistas y busqué, pero nunca pude colocar en su orden la sintaxis correcta.
```